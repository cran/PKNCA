...
The two figures below illustrate the integration with AUC~0-$\infty$,obs~ and AUC~0-$\infty$,pred$.  The difference between the tow figures is most evident at time=8 where there is a discontinuity in integration at `tlast` due to using `clast.pred` after that point and `clast.obs` before that point.  (To illustrate the integration differences, BLOQ indicator shapes have been removed.  BLOQ is handled identically to previous figures.)
